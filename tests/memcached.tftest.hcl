...
# MEMCACHED_SERVERS cannot be asserted literally — the e2e suite covers the
...
run "default_size_is_mini" {
  ...
  assert {
    ...
    error_message = "env contract should expose MEMCACHED_SERVERS"
  }
...
run "modifications_can_be_deferred_to_the_maintenance_window" {
  ...
  assert {
    condition     = !output.apply_immediately
    error_message = "a node replacement should be deferrable to the maintenance window"
  }
